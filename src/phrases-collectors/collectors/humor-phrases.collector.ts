...
const URL = 'https://www.pensador.com/frases_de_humor/';

@Injectable()
export class HumorPhrasesCollector implements PhrasesCollector {
  ...
  async collectData(): Promise<IParseDataResponse[]> {
    ...
    const parsedData = this.parseData(scrapedData);
    const filteredPhrases = filterDuplicatedData(parsedData);
